...
class FeatureTestAllTypesMapper extends Mapper<Void, HAWQRecord, Text, Text> {
	...
	protected void map(Void key, HAWQRecord value, Context context)
		...
		try {
			...
			values.add(fieldToString(value.getTime(14)));
			values.add(fieldToString(value.getTimestamp(15)));
			values.add(fieldToString(value.getTimestamp(16)));
			values.add(fieldToString(value.getInterval(17)));
			values.add(fieldToString(value.getPoint(18)));
			values.add(fieldToString(value.getLseg(19)));
			values.add(fieldToString(value.getBox(20)));
			values.add(fieldToString(value.getCircle(21)));
			values.add(fieldToString(value.getPath(22)));
			values.add(fieldToString(value.getPolygon(23)));
			values.add(fieldToString(value.getMacaddr(24)));
			values.add(fieldToString(value.getInet(25)));
			values.add(fieldToString(value.getCidr(26)));
			values.add(fieldToString(value.getString(27)));
