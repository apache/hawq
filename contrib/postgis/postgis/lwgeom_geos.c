...
#if POSTGIS_GEOS_VERSION >= 33
{
	...
	*/
	...
	if ( DatumGetPointer(datum) == NULL ) PG_RETURN_NULL();
...
Datum pointonsurface(PG_FUNCTION_ARGS)
{
	...
	{
		lwg = lwpoint_as_lwgeom(lwpoint_construct_empty(gserialized_get_srid(geom), gserialized_has_z(geom), gserialized_has_m(geom)));
