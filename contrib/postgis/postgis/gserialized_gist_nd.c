...
Datum gserialized_gist_picksplit(PG_FUNCTION_ARGS)
{
	...
	*/
	...
	POSTGIS_DEBUGF(4, "[GIST] spl_ldatum: %s", gidx_to_string((GIDX*)DatumGetPointer(v->spl_ldatum)));
	POSTGIS_DEBUGF(4, "[GIST] spl_rdatum: %s", gidx_to_string((GIDX*)DatumGetPointer(v->spl_rdatum)));
	...
	POSTGIS_DEBUGF(4, "[GIST] axis %d: parent range (%.12g, %.12g) left range (%.12g, %.12g), right range (%.12g, %.12g)",
	               ...
	               GIDX_GET_MIN((GIDX*)DatumGetPointer(v->spl_ldatum), direction), GIDX_GET_MAX((GIDX*)DatumGetPointer(v->spl_ldatum), direction),
	               GIDX_GET_MIN((GIDX*)DatumGetPointer(v->spl_rdatum), direction), GIDX_GET_MAX((GIDX*)DatumGetPointer(v->spl_rdatum), direction) );
