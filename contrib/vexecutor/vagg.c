...
Datum vfloat8_accum(PG_FUNCTION_ARGS)
{
	...
	vectransdata->data = Float8GetDatum(sum);
	return PointerGetDatum(vectransdata);
}
				   ...
				   MemoryManagerContainer *mem_manager)
{
	...
	for (aggno = 0; aggno < aggstate->numaggs; aggno++)
	{
		...
		slot = ExecVProject(peraggstate->evalproj, NULL);
...
agg_hash_initial_1pass(AggState *aggstate)
{
	...
	while(true)
	{
		...
		memset(&agg_groupdata, 0, sizeof(agg_groupdata));
		memset(&(agg_groupdata.idx_list), -1, 1024 * sizeof(int32));
