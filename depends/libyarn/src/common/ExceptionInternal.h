 ...
 * If this function return true, YarnCanceled will be thrown.
 */
 ...
 * @throw return false if operation is not canceled, else throw YarnCanceled.
 * @throw YarnCanceled
 */
...
#define THROW(throwable, fmt, ...) \
    Yarn::Internal::ThrowException<throwable>(false, __FILE__, __LINE__, #throwable, fmt, ##__VA_ARGS__);

#define NESTED_THROW(throwable, fmt, ...) \
    Yarn::Internal::ThrowException<throwable>(true, __FILE__, __LINE__, #throwable, fmt, ##__VA_ARGS__);
