...
void RpcChannelImpl::sendSaslMessage(hadoop::common::RpcSaslProto * msg, Message * resp) {
    int totalLen;
    WriteBuffer buffer;
...
const hadoop::common::RpcSaslProto_SaslAuth * RpcChannelImpl::createSaslClient(
    const RepeatedPtrField<hadoop::common::RpcSaslProto_SaslAuth> * auths) {
    const hadoop::common::RpcSaslProto_SaslAuth * auth = NULL;
    Token token;
...
std::string RpcChannelImpl::saslEvaluateToken(hadoop::common::RpcSaslProto & response, bool serverIsDone) {
    std::string token;
...
RpcAuth RpcChannelImpl::setupSaslConnection() {
    RpcAuth retval;
    hadoop::common::RpcSaslProto negotiateRequest, response, msg;
    negotiateRequest.set_state(hadoop::common::RpcSaslProto_SaslState_NEGOTIATE);
    ...
    do {
        ...
        case hadoop::common::RpcSaslProto_SaslState_NEGOTIATE: {
            const hadoop::common::RpcSaslProto_SaslAuth * auth = createSaslClient(
                    &response.auths());
            ...
            if (retval.getMethod() == AuthMethod::SIMPLE) {
                ...
                hadoop::common::RpcSaslProto_SaslAuth * respAuth = msg.add_auths();
                ...
                msg.set_state(hadoop::common::RpcSaslProto_SaslState_INITIATE);
            }

            break;
        }

        case hadoop::common::RpcSaslProto_SaslState_CHALLENGE: {
            if (!saslClient) {
            ...
            msg.set_token(token);
            msg.set_state(hadoop::common::RpcSaslProto_SaslState_RESPONSE);
            break;
        }

        case hadoop::common::RpcSaslProto_SaslState_SUCCESS:
            if (!saslClient) {
...
exception_ptr RpcChannelImpl::invokeInternal(RpcRemoteCallPtr remote) {
    ...
    try {
        ...
        try {
            NESTED_THROW(YarnFailoverException,
...
void RpcChannelImpl::checkOneResponse() {
    ...
    while (client.isRunning()) {
        ...
        if (timeout > 0 && ToMilliSeconds(start, steady_clock::now()) >= timeout) {
            try {
                THROW(Yarn::YarnTimeoutException, "Timeout when wait for response from RPC channel \"%s:%s\"",
                      key.getServer().getHost().c_str(), key.getServer().getPort().c_str());
            } catch (...) {
                NESTED_THROW(Yarn::YarnRpcException, "Timeout when wait for response from RPC channel \"%s:%s\"",
                             key.getServer().getHost().c_str(), key.getServer().getPort().c_str());
            }
        }
    }
...
void RpcChannelImpl::buildConnectionContext(
    ...
    if (auth.getMethod() != AuthMethod::TOKEN) {
        hadoop::common::UserInformationProto * user = connectionContext.mutable_userinfo();
...
void RpcChannelImpl::readOneResponse(bool writeLock) {
    ...
    if (hadoop::common::RpcResponseHeaderProto_RpcStatusProto_SUCCESS == status) {
        /*
        ...
        if (hadoop::common::RpcResponseHeaderProto_RpcStatusProto_ERROR == status) {
            RpcRemoteCallPtr rc;
            {
                lock_guard<mutex> lock(writeMut);
                rc = getPendingCall(curRespHeader.callid());
            ...
            try {
                THROW(YarnRpcServerException, "%s: %s",
                      errClass.c_str(), errMessage.c_str());
            } catch (YarnRpcServerException & e) {
                e.setErrClass(errClass);
                e.setErrMsg(errMessage);
                rc->cancel(HandlerRpcResponseException(current_exception()));
            }
        } else { /*fatal*/
            assert(hadoop::common::RpcResponseHeaderProto_RpcStatusProto_FATAL == status);

            if (errClass.empty()) {
                THROW(YarnRpcException, "%s: %s",
                      errClass.c_str(), errMessage.c_str());
            }

            try {
                THROW(YarnRpcServerException, "%s: %s", errClass.c_str(),
                      errMessage.c_str());
            } catch (YarnRpcServerException & e) {
                e.setErrClass(errClass);
                e.setErrMsg(errMessage);
