...
* new
*
* Revision 1.5  2007/09/12 21:55:05  cmcdevitt
* fix a lot of incorrect format specifiers, and fix some 64-bit issues
*
* Revision 1.4  2007/04/10 00:08:54  cmcdevitt
...
#define LIFENOISE(n, var)	\
	if (o_verbose > 0) fprintf(stderr, "%c\b", lnoise[(var%LN_CNT)])
...
EXTERN long o_scale;
EXTERN int o_refresh;
EXTERN int o_resume;
EXTERN long o_verbose;
EXTERN long o_force;
EXTERN long o_header;
EXTERN long o_columnar;
EXTERN long o_direct;
EXTERN long o_updates;
EXTERN long o_table;
EXTERN long o_fnames;
EXTERN int  o_gen_sql;
EXTERN int  o_gen_rng;
EXTERN char *o_db_name;
EXTERN char *o_schema_name;
EXTERN int  o_step;
EXTERN long o_stepmax;
EXTERN int	o_set_seeds;
EXTERN int  o_validate;
EXTERN char *o_d_path;
...
/*
 * Cdb usage additions.
 * numSegs is the number of segments in the cluster,
 * used in the makeCdbHash API function,
 * and segIndexFlags is an array of length numSegs that holds a flag indicating
 * whether that segIndex is a desired result from cdbhashreduce.
 * If the row doesn't hashreduce to segIndex, it doesn't get stored.
 */
EXTERN int	numSegs;
EXTERN int	*segIndexFlags;

EXTERN int  isPartitioned;
...
#define  PS_SKEY_MAX  ((tdefs[SUPP].base - 1) * o_scale)
...
#define  O_SIZE          109
#define  O_CKEY_MIN      1
#define  O_CKEY_MAX      (tdefs[CUST].base * o_scale)
...
#define  L_PKEY_MIN   1
#define  L_PKEY_MAX   (tdefs[PART].base * o_scale)
#define  L_SDTE_MIN   1
