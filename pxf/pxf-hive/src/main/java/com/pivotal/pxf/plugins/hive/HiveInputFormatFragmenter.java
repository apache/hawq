 ...
 * This is done to avoid memory explosion in Hawq.
 * For RC use together with HiveRCFileAccessor/HiveColumnarSerdeResolver.
 * For Text use together with HiveLineBreakAccessor/HiveStringPassResolver.
 * <p/>
...
public class HiveInputFormatFragmenter extends HiveDataFragmenter {
     ...
     */
    static public String[] parseToks(InputData input, String... supportedSerdes) throws Exception {
        String userData = new String(input.getFragmentUserData());
        String[] toks = userData.split(HIVE_UD_DELIM);
        if (supportedSerdes.length > 0 && !Arrays.asList(supportedSerdes).contains(toks[TOK_SERDE])) {
            throw new UserDataException(toks[TOK_SERDE] + " serializer isn't supported by " + input.getAccessor());
        }
     ...
     */
    private String assertFileType(String className, HiveTablePartition partData) throws Exception {
        switch (className) {
            ...
            default:
                throw new IllegalArgumentException("HiveInputFormatFragmenter does not yet support " + className +
                        " for " + partData + ". Supported InputFormat are " + Arrays.toString(PXF_HIVE_INPUT_FORMATS.values()));
        }
    }
        ...
        switch (className) {
            ...
            default:
                throw new UnsupportedTypeException("HiveInputFormatFragmenter does not yet support  " + className +
                        " for " + partData + ". Supported serializers are: " + Arrays.toString(PXF_HIVE_SERDES.values()));
        }
    }
