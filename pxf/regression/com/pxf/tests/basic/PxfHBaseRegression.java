...
import com.pivotal.parot.structures.tables.pxf.ExternalTable;
...
public class PxfHBaseRegression extends PxfTestCase {
	...
	public void rowRange() throws Exception {
		...
		hbase.queryResults(hTable, null);
		hawq.queryResults(externalTableHbase, "SELECT * FROM " + externalTableHbase.getName() + whereClause + " ORDER BY recordkey ASC");
	...
	public void specificRow() throws Exception {
		...
		hbase.queryResults(hTable, null);
		hawq.queryResults(externalTableHbase, "SELECT * FROM " + externalTableHbase.getName() + whereClause);
	...
	public void filterSeveralQualifiers() throws Exception {
		...
		/**
		 * TODO: pending adding: BYTEA support to filter pushdown (q4) cast between text and varchar
		 * (cf1:q1)
		 */
	...
	public void filterTextAndNumeric() throws Exception {
		...
		/**
		 * TODO: failing test - UTF-8 encoding isn't passed correctly to PXF. see GPSQL-1739.
		 */
		// createAndQueryPxfHBaseFilterTable(filterString);
		// createAndQueryPxfHawqFilterTable(hTable, exTableFields, whereClause, filterString);
	}
	...
	public void filterOrAnd() throws Exception {
		...
		String whereClause = " WHERE (((recordkey > 'row00000090') AND (recordkey <= 'row00000103'))" + " OR (recordkey = 'row00000105'))";
		String filterString = NO_FILTER;
		...
		whereClause = " WHERE (recordkey = 'row00000105') OR " + "((recordkey > 'row00000090') AND (recordkey <= 'row00000103'))";
		createAndQueryPxfHawqFilterTable(hTable, exTableFields, whereClause, filterString);

		ReportUtils.report(report, getClass(), "Query: (x AND (y OR z)). Expected filter: x");
		whereClause = " WHERE (recordkey > 'row00000090') AND " + "((recordkey <= 'row00000103') OR (recordkey = 'row00000105'))";
		filterString = "a0c\"row00000090\"o2";
		createAndQueryPxfHawqFilterTable(hTable, exTableFields, whereClause, filterString);

		ReportUtils.report(report, getClass(), "Query: (x AND (y AND z OR a)). Expected filter: x");
		whereClause = " WHERE (recordkey > 'row00000090') AND " + "((recordkey <= 'row00000103') AND (recordkey = 'row00000105') OR recordkey = '0')";
		filterString = "a0c\"row00000090\"o2";
		createAndQueryPxfHawqFilterTable(hTable, exTableFields, whereClause, filterString);
	...
	public void filterAndNotEquals() throws Exception {
		...
		String whereClause = " WHERE recordkey != 'row00000099' AND \"cf1:q8\" > 97 AND \"cf1:q9\" <= 9702990000000099";
	...
	public void nullValues() throws Exception {
		...
		hawq.queryResults(exTable, "SELECT * FROM " + exTable.getName() + whereClause + " ORDER BY recordkey ASC");
	...
	public void noLookupTable() throws Exception {
		...
		ReportUtils.reportBold(report, getClass(), "Drop lookup table, fail to query external table because of q4 field resolve");
	...
	public void recordKeyAsInteger() throws Exception {
		...
		hawq.queryResults(exTableIntegerRowKey, "SELECT * FROM " + exTableIntegerRowKey.getName() + whereClause);
		...
		hawq.queryResults(exTableIntegerRowKey, "SELECT * FROM " + exTableIntegerRowKey.getName() + whereClause + " ORDER BY recordkey;");
	...
	/**
	 * Making sure the required table is exists, if not creating it and populate it.
	 * 
	 * @param table
	...
	/**
	 * Creates and queries table using HBaseAccessorWithAccessor. Table fields are
	 * {@link exTableFields}, HBase table in the path is {@link hTable}, and is also used for
	 ...
	 * @throws Exception
	 */
	private void createAndQueryPxfHBaseFilterTable(String filter, HBaseTable hbaseTable, String[] fields)
			throws Exception {
	...
	private void createAndQueryPxfHawqFilterTable(HBaseTable hbaseTable, String[] fields, String whereClause, String expectedFilter)
			throws Exception {

		ReportUtils.report(report, getClass(), "running query on FilterPrinterAccessor " + "with WHERE: clause " + whereClause + ", expected filter: " + expectedFilter);
