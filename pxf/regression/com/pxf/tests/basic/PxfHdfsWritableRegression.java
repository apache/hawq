...
public class PxfHdfsWritableRegression extends PxfTestCase {
	...
	static String[] syntaxFields = new String[] {
		"a int", 
		"b text", 
		"c bytea"
	};
	...
	public void syntaxValidation() throws Exception {
		...
		weTable.setAccessor("SequenceFileAccessor");
		weTable.setResolver("AvroResolver");
	...
	/**
	 * Create writable table with no accessor and resolver.
	 * Should fail and throw the right message.
	 * 
	 * @throws Exception
	 */
	@Test
	...
	/**
	 * Create writable table with no parameters.
	 * Should fail and throw the right message.
	 * 
	 * @throws Exception
	 */
	...
	public void textInsertFromRead() throws Exception {
		...
		hawq.runQuery("INSERT INTO " + writableTable.getName() 
		              + " SELECT * FROM " + readableTable.getName() + " WHERE n1<=10", false);
		hawq.runQuery("INSERT INTO " + writableTable.getName() 
		              + " SELECT * FROM " + readableTable.getName() + " WHERE n2<100", false);
	...
	/**
	 * Create writable table with Default codec, insert data to it, then
	 * read it from HDFS using Readable Table.
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * Create writable table with BZip2 codec, insert data to it using copy, than
	 * read it from HDFS using Readable Table.
	 * 
	 * @throws Exception
	 */
	...
	public void bzip2Text() throws Exception {
		...
		hawq.runQueryWithExpectedWarning("CREATE TEMP TABLE " + tempTable.getName() + " (LIKE " + weTable.getName() + ")",
	...
	public void sequence() throws Exception {
		...
		for (String codec : codecs) {
			for (String[] userParam : userParams) {
				...
				String testDescription = "sequence test #" + testNum +
						": with " +
						((codec == null) ? "no" : codec) + " codec" +
						". user params:" + 
						((userParam == null) ? "null" : ArrayUtils.toString(userParam));
	...
	public void negativeWrongPort() throws Exception {
		weTable = new WritableExternalTable("wr_wrong_port", 
		                                    new String[] { "t1 text",
														   "a1 integer" },
											hdfsWorkingFolder + "/writable/err", "TEXT");
		...
		try {
			...
			ExceptionUtils.validate(report, e, new PSQLException("ERROR: remote component error \\(0\\): " +
					"(Failed connect to " + weTable.getHostname() +":12345; Connection refused|couldn't connect to host).*?", null), true);
		}
	}
	...
	public void negativeBadHost() throws Exception {
		weTable = new WritableExternalTable("wr_host_err", 
		                                    new String[] { "t1 text",
														   "a1 integer" },
											hdfsWorkingFolder + "/writable/err", "TEXT");
		weTable.setDelimiter(",");
		...
		try {
			...
			ExceptionUtils.validate(report, e, new PSQLException("ERROR: remote component error \\(0\\): " +
					"Couldn't resolve host 'badhostname'.*?", null), true);
		}
	}
	...
	public void circleType() throws Exception {
		...
		weTable.setAccessor("SequenceFileAccessor");
		weTable.setResolver("WritableResolver");
		...
		reTable.setFragmenter("HdfsDataFragmenter");
		reTable.setAccessor("SequenceFileAccessor");
		reTable.setResolver("WritableResolver");
	...
	public void negativeCharType() throws Exception {
		...
		weTable = new WritableExternalTable("wr_host_err", 
		                                    new String[] { "a1 integer",
														   "c1 char" },
											hdfsWorkingFolder + "/writable/err", "CUSTOM");
		weTable.setAccessor("SequenceFileAccessor");
		weTable.setResolver("WritableResolver");
		...
		try {
			...
			ExceptionUtils.validate(report, e, new PSQLException("ERROR: remote component error \\(500\\) from " +
					"'.*?': " +
					"Problem accessing /gpdb/v(\\d+)/Writable/stream. Reason:(\\s+)" +
					"Server Error(\\s+)Caused by:(\\s+)com.pivotal.pxf.exception.BadRecordException: " + 
					"com.pivotal.pxf.exception.UnsupportedTypeException: Type char is not supported by GPDBWritable.*?", null), true);
		}
	}
	...
	public void analyzeWithoutAnalyzer() throws Exception {
		...
		weTable = new WritableExternalTable("writable_analyze_no_analyzer", 
		                                    fields,
											hdfsWorkingFolder + "/writable/analyze/noanalyzer", "TEXT");
	...
	public void analyzeWithAnalyzer() throws Exception {
		...
		weTable = new WritableExternalTable("writable_analyze_with_analyzer", 
		                                    fields,
											hdfsWorkingFolder + "/writable/analyze/analyzer", "TEXT");
		weTable.setAccessor("LineBreakAccessor");
		weTable.setResolver("StringPassResolver");
	...
	public void negativeCompressionTypeNone() throws Exception{
		...
		weTable = TableFactory.getPxfWritableSequenceTable("compress_type_none", fields, 
		                                                   hdfsWorkingFolder + "/writable/err", "SomeClass");
		...
		try {
			...
			ExceptionUtils.validate(report, e, new PSQLException("ERROR: remote component error \\(500\\) from " +
					"'.*?': " +
					"Problem accessing /gpdb/v(\\d+)/Writable/stream. Reason:(\\s+)" +
					"Server Error(\\s+)Caused by:(\\s+)java.lang.IllegalArgumentException: " +
					"Illegal compression type 'NONE'\\. For disabling compression remove COMPRESSION_CODEC parameter\\..*?", null), true);
		}
	}
	...
	public void csv() throws Exception {
		...
		weTable = new WritableExternalTable("writable_csv", 
		                                    fields,
											hdfsDir, "CSV");
		weTable.setProfile("HdfsTextSimple");
		...
		reTable = new ReadableExternalTable("readable_csv",
		                                   fields,
		                                   hdfsDir, "CSV");
	...
	public void threadSafe() throws Exception {
		...
		weTable = new WritableExternalTable("writable_threadsafe", 
		                                    fields,
											hdfsDir, "CSV");
		weTable.setProfile("HdfsTextSimple");
		...
		reTable = new ReadableExternalTable("readable_threadsafe",
		                                   fields,
		                                   hdfsDir, "CSV");
		reTable.setProfile("HdfsTextSimple");
	...
	public void recordkeyIntType() throws Exception {
		...
		String copyCmd = "COPY " + weTable.getName() + " FROM '" + path.getAbsolutePath() + "'" +
	...
	/**
	 * create data file based on CustomWritable schema.
	 * data is written from sudoDataTable and to path file.
	 * @param sudoDataTable
	 * @param path
	 * @param recordkey add recordkey data to the beginning of each row
	 * @throws IOException
	 */
	private void createCustomWritableDataFile(Table sudoDataTable, File path, boolean recordkey) throws IOException {
