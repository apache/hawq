...
public final class CompressQuickLZ {
			...
			if (level == 1) {
				...
				if (cache == 0
					...
					if (source[o + 3] != source[src + 3]) {
						...
						int old_src = src;
						int remaining = ((source.length - UNCOMPRESSED_END
								- src + 1 - 1) > 255 ? 255 : (source.length
				...
				int matchlen, k, m;
				byte c;
				int remaining = ((source.length - UNCOMPRESSED_END - src + 1 - 1) > 255 ? 255
				...
				if (dst <= last_matchstart) {
					...
					if (level == 1) {
						while (last_hashed < dst - 3) {
							++last_hashed;
							int fetch2 = (int) fast_read(destination,
									last_hashed, 3);
