...
=head1 VERSION
 ...
 This document describes version 31 of tidycat.pl, released
 Tue Aug  9 15:47:46 2011.
...
BEGIN {
	...
	$glob_glob->{_sleazy_properties}->{version} = '31';
	$glob_glob->{_sleazy_properties}->{COPYDATES} = '2009-2011';
	$glob_glob->{_sleazy_properties}->{slzy_date} = '1312930065';
...
sub getcomment2
{
	...
	my $bigstr = <<"EOF_bigstr";
/* ----------------
 *TWOTABSForm_TABLENAME corresponds to a pointer to a tuple with
...
#	print $coltype, "\n";
	...
	unless ($coltype =~ m/^(quoted\_char|name|oid|boolean|real|smallint|integer|timestamp\_with\_time\_zone)(\[\])?$/)
	{
		$coltype =~ s/\[\]/\[1\]/;
		return $coltype;
	}

	$ctype = "NameData" if ($coltype =~ m/name/);
	$ctype = "Oid" if ($coltype =~ m/oid/);
	$ctype = "bool" if ($coltype =~ m/boolean/);
	$ctype = "float4" if ($coltype =~ m/real/);
	$ctype = "int2" if ($coltype =~ m/smallint/);
	$ctype = "int4" if ($coltype =~ m/integer/);
	$ctype = "timestamptz" if ($coltype =~ m/timestamp\_with\_time\_zone/);

	# Note: a quoted_char (or "char") is a single C char
	$ctype = "char" if ($coltype =~ m/quoted\_char/);

	# make an array of 1
	$ctype .= "[1]" if ($coltype =~ m/\[\]$/);
...
sub parsefk
{
	...
	my @baz = ($bigstr =~ m/^\s*alter\s+table\s+(\w+)\s+add\s+fk/i);

	my $tname = shift @baz;

	die "$filnam: bad foreign key - no such table: $tname" unless (exists($alltabs->{$tname}));

	$bigstr =~ s/^\s*alter\s+table\s+\w+\s+add\s+fk//i;
...
sub formatcolconst
{
	...
	for my $coldef (@{$allcol})
	{
		...
		$cdef = "#define Anum_" . $tnamepref . "_" . $coldef->{colname};
...
sub formattab
{
	...
	for my $kk (@{$keys})
	{
		...
		$bigstr .= getcomment2($kk);

		$bigstr .= "typedef FormData_" . $kk . " *Form_" . $kk. ";\n";
	...
	if ($glob_glob->{dumpdef})
	{
		...
		if ($glob_glob->{dumpformat} =~ m/j/)
		{
			if (eval "require JSON")
			{
				...
				print $file_out JSON::to_json(\%dumptabs_h, 
												{pretty => 1, indent => 2, 
												 canonical => 1});
			}
			else
