...
# Generated by sleazy.pl version 5 (release Wed Jul 18 19:18:45 2012)
...
=head1 VERSION
 ...
 This document describes version 6 of sleazy.pl, released
 Mon Aug 20 12:30:03 2012.
...
Copyright (c) 2012 Greenplum.  All rights reserved.  
...
sub glob_validate
{
	...
	if (-e $proplistfil)
	{
		...
		{
			$glob_glob->{_sleazy_properties}->{$kk} = $vv;
		}
	}
...
BEGIN {
	...
	$glob_glob->{_sleazy_properties}->{version} = '6';
	$glob_glob->{_sleazy_properties}->{slzy_date} = '1345491003';
...
sub get_derived_properties
{
	...
	$copyholder = (exists($allprops->{COPYHOLDER})) ?
...
#			((length($lng) < 72) ? $glob_glob2->{tabstr} : "" ) .
	...
	{
		while (my ($kk, $vv) = each(%{$glob_glob->{_sleazy_properties}}))
		{
			$allprops{$kk} = $vv;
		}
	}
