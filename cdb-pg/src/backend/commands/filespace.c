...
/*
 * AlterFilespaceSetLocation
 * 		Change filesspace location
 */
void
AlterFilespaceSetLocation(AlterFileSpaceStmt *stmt)
{
	char             *fsname;
	Oid               fsoid;
	Relation	      rel;
	...
	if (!OidIsValid(fsForm->fsfsys))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_NAME),
				 errmsg("permission denied: \"%s\" is a system filespace on local file system",
						fsname)));
	...
	/* scan and update pg_filespace_entry */
	...
	newlen = strlen(stmt->location);

	while (HeapTupleIsValid(tup))
	{
		Datum		values[Natts_pg_filespace_entry];
		bool		nulls[Natts_pg_filespace_entry];
		bool		replace[Natts_pg_filespace_entry];
		HeapTuple	newtuple;
		char	   *newlocation, *oldlocation, *segprefix;
		Form_pg_filespace_entry entry;
		int 		oldlen;

		memset(nulls, false, sizeof(nulls));
		memset(replace, false, sizeof(replace));

		entry = (Form_pg_filespace_entry) GETSTRUCT(tup);

		/*
		 * skip first database, its location is on local filesystem
		 */
		if (entry->fsedbid == 1)
		{
			tup = caql_getnext(pcqCtx);
		...
		oldlocation = text_to_cstring(&entry->fselocation);
		oldlen = strlen(oldlocation);

		static char PATH_SEP = '/';

		for (segprefix = oldlocation + oldlen - 1 ; segprefix != oldlocation ; --segprefix)
		{
			if (*segprefix == PATH_SEP && segprefix != oldlocation + oldlen - 1)
				break;
		}

		Assert(*segprefix == PATH_SEP);

		newlocation = palloc(strlen(segprefix) + newlen + 1);
		sprintf(newlocation, "%s%s", stmt->location, segprefix);

		replace[Anum_pg_filespace_entry_fselocation - 1] = true;
		values[Anum_pg_filespace_entry_fselocation - 1] = CStringGetTextDatum(newlocation);
