...
#include "storage/quicklz1.h"
	...
	#error quicklz1.c must be compiled with compression level 1
#endif
...
int qlz1_get_setting(int setting)
{
	switch (setting)
	{
		case 0: return QLZ_COMPRESSION_LEVEL;
		case 1: return sizeof(qlz1_state_compress);
		case 2: return sizeof(qlz1_state_decompress);
...
static void reset_table_compress(qlz1_state_compress *state)
{
	int i;
...
static void reset_table_decompress(qlz1_state_decompress *state)
{
	int i;
	(void)state;
	(void)i;
...
size_t qlz1_size_decompressed(const char *source)
{
	ui32 n, r;
	n = (((*source) & 2) == 2) ? 4 : 1;
...
size_t qlz1_size_compressed(const char *source)
{
	ui32 n, r;
	n = (((*source) & 2) == 2) ? 4 : 1;
...
static __inline
size_t qlz1_size_header(const char *source)
{
...
static __inline void update_hash(qlz1_state_decompress *state, const unsigned char *s)
{
#if QLZ_COMPRESSION_LEVEL == 1
	ui32 hash;
	hash = hashat(s);
...
#if QLZ_COMPRESSION_LEVEL <= 2
static void update_hash_upto(qlz1_state_decompress *state, unsigned char **lh, const unsigned char *max)
{
	while(*lh < max)
	{
		(*lh)++;
...
static size_t qlz1_compress_core(const unsigned char *source, unsigned char *destination, size_t size, qlz1_state_compress *state)
{
	...
	// min. size must be 9 bytes so that the qlz1_size functions can take 9 bytes as argument
	return dst - destination < 9 ? 9 : dst - destination;
}

static size_t qlz1_decompress_core(const unsigned char *source, unsigned char *destination, size_t size, qlz1_state_decompress *state, const unsigned char *history)
{
	const unsigned char *src = source + qlz1_size_header((const char *)source);
	...
	const unsigned char *last_source_byte = source + qlz1_size_compressed((const char *)source) - 1;
...
size_t qlz1_compress(const void *source, char *destination, size_t size, qlz1_state_compress *state)
{
	size_t r;
	ui32 compressed;
	size_t base;
...
#endif
	{
		...
		r = base + qlz1_compress_core((const unsigned char *)source, (unsigned char*)destination + base, size, state);
#if QLZ_STREAMING_BUFFER > 0
		reset_table_compress(state);
#endif
		if(r == base)
	...
	else
	{
		...
		r = base + qlz1_compress_core(src, (unsigned char*)destination + base, size, state);
...
size_t qlz1_decompress(const char *source, void *destination, qlz1_state_decompress *state)
{
	size_t dsiz = qlz1_size_decompressed(source);

#if QLZ_STREAMING_BUFFER > 0
	if (state->stream_counter + qlz1_size_decompressed(source) - 1 >= QLZ_STREAMING_BUFFER)
#endif
	{
		if((*source & 1) == 1)
		{
			reset_table_decompress(state);
			dsiz = qlz1_decompress_core((const unsigned char *)source, (unsigned char *)destination, dsiz, state, (const unsigned char *)destination);
		}
		else
		{
			memcpy(destination, source + qlz1_size_header(source), dsiz);
		}
		state->stream_counter = 0;
		reset_table_decompress(state);
	}
	...
	else
	{
		...
		if((*source & 1) == 1)
		{
			dsiz = qlz1_decompress_core((const unsigned char *)source, dst, dsiz, state, (const unsigned char *)state->stream_buffer);
		}
		else
		{
			memcpy(dst, source + qlz1_size_header(source), dsiz);
			reset_table_decompress(state);
		}
