...
/**
 * Read schema element, field information of parquet file
...
readSchemaElement(
		...
		int *schemaTreeNodeCount)
{
	int maxFieldCount = schemaListSize - 1;
	*pfields =
	...
	for (int i = 1; i < schemaListSize; i++) {
		readParquetSchema_SingleField(&((*pfields)[fieldIndex]), prot, &i, r, d,
				depth, colCount, schemaTreeNodeCount, NULL);
		fieldIndex++;
		if (fieldIndex > maxFieldCount) {
			ereport(ERROR,
					(errcode(ERRCODE_GP_INTERNAL_ERROR),
							errmsg("file metadata schema element information not correct")));
		}
	}
		...
		CompactProtocol **prot)
{
	...
	/*Free the footer protocol*/
	freeCompactProtocol(*prot);
	pfree(*prot);
}
