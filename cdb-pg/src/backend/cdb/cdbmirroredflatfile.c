...

{
	...
	if (FileRepPrimary_BypassMirrorCheck(open->dbDirNode.tablespace, InvalidOid, NULL))
		mirrorFlush = FALSE;
...
PgVersionRecoverMirror(void)
{
											 ...
											 &persistentSerialNum))
	{
		...
		while (1) {
			...
			/* If this system is not a local tablespace, we don't check the file. */
			if (strstr(subDirectoryMirror, "://") != NULL)
				break;
