	...
	)
{
	...
	Plan *pplan = PplFromDXL(pdxln, &dxltrctx, NULL);
	...
	)
{
	...
	for (ULONG ulLoc = 0; ulLoc < ulLocations; ulLoc++)
	{
		const CHAR *szUri = CTranslatorUtils::SzFromWsz(pmdrelext->PstrLocation(ulLoc)->Wsz());
	...
	)
{
	...
	while (!fDone)
	{
		for (ULONG ulLoc = 0; ulLoc < ulLocations; ulLoc++)
		{
			CHAR *szUri = CTranslatorUtils::SzFromWsz(pmdrelext->PstrLocation(ulLoc)->Wsz());
	...
	ForEach (plc, plModifiedLocations)
	{
		...
		pUri = gpdb::PuriParseExternalTable(szUri);
...
{
	const CHAR *szCommand = CTranslatorUtils::SzFromWsz(pmdrelext->PstrCommand()->Wsz());
	...
	CHAR *szOnClause = CTranslatorUtils::SzFromWsz(pmdrelext->PstrLocation(0)->Wsz());
	...
	)
{
	...
	if (!fUsingExecute)
	{
		szFirstUri = CTranslatorUtils::SzFromWsz(pmdrelext->PstrLocation(0)->Wsz());
		pUri = gpdb::PuriParseExternalTable(szFirstUri);
	}

	if (fUsingLocation && (URI_FILE == pUri->protocol || URI_HTTP == pUri->protocol))
	{
		MapLocationsFile(pmdrelext, rgszSegFileMap, pcdbCompDB);
	}
	...
	{
		MapLocationsExecute(pmdrelext, rgszSegFileMap, pcdbCompDB, ulTotalPrimaries);
	}
	else
	{
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorDXLToPlStmt::CExternalScanFormatType
//
//	@doc:
...
CTranslatorDXLToPlStmt::CExternalScanFormatType
	(
	IMDRelationExternal::Erelextformattype erelextformat
	)
{
	switch (erelextformat)
	{
		case IMDRelationExternal::ErelextformatCSV:
			return 'c';
		case IMDRelationExternal::ErelextformatText:
			return 't';
		case IMDRelationExternal::ErelextformatCustom:
			return 'b';
		default:
			GPOS_ASSERT(!"Unrecognized format type");
			return '\0';
	}
}
//---------------------------------------------------------------------------
...
{
	// translate table descriptor into a range table entry
	CDXLPhysicalLimit *pdxlopLimit = CDXLPhysicalLimit::PdxlopConvert(pdxlnLimit->Pdxlop());
	...
	Plan *pplanLeft = PplFromDXL(pdxlnChildPlan, &dxltrctxLeft, pplan);
	...
	)
{
	...
	Plan *pplanRight = (Plan*) PhhashFromDXL(pdxlnRight, &dxltrctxRight, pplan);
	...
	)
{
	...
	Plan *pplanLeft = PplFromDXL(pdxln, &dxltrctx, pplan);
	...
	)
{
	...
	{
		return PplanResultHashFilters(pdxlnMotion, pdxltrctxOut, pplanParent);
	}
	
	return PplanMotionFromDXLMotion(pdxlnMotion, pdxltrctxOut, pplanParent);
}
	...
	)
{
	...
	Plan *pplanChild = PplFromDXL(pdxlnChild, &dxltrctxChild, pplan);
	...
	)
{
	...
	CDXLPhysicalResult *pdxlopResult = CDXLPhysicalResult::PdxlopConvert(pdxlnResult->Pdxlop());
	...
	)
{
	...
	if (EdxlshscanIndexSentinel == pdxlnSharedScan->UlArity())
	{
		...
		Plan *pplanChild = PplFromDXL(pdxlnChild, pdxltrctxChild, pplan);
	...
	)
{
	...
	CDXLPhysicalSequence *pdxlop = CDXLPhysicalSequence::PdxlopConvert(pdxlnSequence->Pdxlop());
	...
	Plan *pplanLastChild = PplFromDXL(pdxlnLastChild, &dxltrctxChild, pplan);
	...
	)
{
	...
	pdts->scan.scanrelid = iRel;
	pdts->partIndex = pdxlop->UlPartIndexId();

	Plan *pplan = &(pdts->scan.plan);
	pplan->plan_node_id = m_pctxdxltoplstmt->UlNextPlanId();
	pplan->plan_parent_node_id = IPlanId(pplanParent);
	pplan->nMotionNodes = 0;
	...
	)
{
	...
	pdis->partIndex = pdxlop->UlPartIndexId();
	...
	)
{
	...
	passert->errmessage = CTranslatorUtils::SzFromWsz(pdxlopAssert->Pstr()->Wsz());
