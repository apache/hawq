...
GRANT SELECT ON test1 TO reg_u1;
...
GRANT SELECT ON test1 TO reg_u2;
...
BEGIN;
DECLARE c1 CURSOR FOR SELECT 1 FROM test1;
DECLARE c2 CURSOR FOR SELECT 2 FROM test1;
DECLARE c3 CURSOR FOR SELECT 3 FROM test1; -- should detect deadlock
END;
...
CLOSE c1;
DECLARE c2 CURSOR FOR SELECT 2 FROM test1;
DECLARE c3 CURSOR FOR SELECT 3 FROM test1;
CLOSE c3;
DECLARE c4 CURSOR FOR SELECT 4 FROM test1;
FETCH c4;
END;
...
BEGIN;
DECLARE c1 CURSOR FOR SELECT * FROM test1;
SELECT rsqname, rsqholders FROM pg_resqueue_status WHERE rsqcostvalue > 0;
DECLARE c2 CURSOR FOR SELECT * FROM test1 a NATURAL JOIN test1 b;
SELECT rsqname, rsqholders FROM pg_resqueue_status WHERE rsqcostvalue > 0;
CLOSE c1;
CLOSE c2;
END;
...
DECLARE c2 CURSOR FOR SELECT * FROM test1;
SELECT rsqname, rsqcostlimit, rsqwaiters, rsqholders FROM pg_resqueue_status WHERE rsqcostvalue > 0;
DECLARE c3 CURSOR FOR SELECT * FROM test1 a, test1 b;
SELECT rsqname, rsqcostlimit, rsqwaiters, rsqholders FROM pg_resqueue_status WHERE rsqcostvalue > 0;
DECLARE c4 CURSOR FOR SELECT * FROM test1 a, test1 b, test1 c;
SELECT rsqname, rsqcostlimit, rsqwaiters, rsqholders FROM pg_resqueue_status WHERE rsqcostvalue > 0;
END;
...
DECLARE c1 CURSOR FOR SELECT 1 FROM test1;
DECLARE c2 CURSOR FOR SELECT 2 FROM test1;
DECLARE c3 CURSOR FOR SELECT 3 FROM test1; -- should not deadlock, we are SU.
END;
