...
* new
*
* Revision 1.4  2007/04/07 22:56:16  cmcdevitt
* Fix stupid bug
*
* Revision 1.3  2007/04/07 08:10:40  cmcdevitt
* Fixes for dbgen with large scale factors
*
* Revision 1.2  2006/06/08 18:51:19  jeshleman
* Modified dbgen so that when using mpp hash for parallel generation, the code
* now exits the gen process early for the customer,supplier, and parts table
* now too.
** Revision 1.1  2006/04/18 23:17:18  cmcdevitt* Add TPC-H code.. dbgen, qgen, and tpchdriver*
*
...
#define PART_SUPP_BRIDGE(tgt, p, s) \
    { \
    DSS_HUGE tot_scnt = tdefs[SUPP].base * o_scale; \
...
mk_order(DSS_HUGE index, order_t *o, long upd_num)
	...
	mk_sparse (index, &o->okey,
		(upd_num == 0) ? 0 : 1 + upd_num / (10000 / o_refresh));


   if (o_scale >= 30000 || O_CKEY_MAX >= 2147483647)
	...
	RANDOM(clk_num, 1, MAX((o_scale * O_CLRK_SCL), O_CLRK_SCL), O_CLRK_SD);
    ...
    if (o_scale >= 10000 || L_PKEY_MAX > 2147483647)
