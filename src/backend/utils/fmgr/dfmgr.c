						  ...
						  const Pg_magic_struct *module_magic_data)
{
	...
	{
		/*
		 * Up to HAWQ 1.2.1.2, our magic number for modules had the product
		 * number encoded as "4.2", as an artifact from GPDB. Don't show
		 * that 4.2 number when detecting those modules as it doesn't make
		 * any sense for HAWQ users. (GPSQL-3083)
		 */
		if (module_magic_data->version == 402)
		{
			ereport(ERROR,
					(errmsg("incompatible library \"%s\": version mismatch",
							libname),
							errdetail("Server version is %s %d.%d, library is from an older version of HAWQ.",
									magic_product,
		...
		else
		{
			ereport(ERROR,
					(errmsg("incompatible library \"%s\": version mismatch",
							libname),
							errdetail("Server version is %s %d.%d, library is %s %d.%d.",
									magic_product,
									magic_data.version / 100,
									magic_data.version % 100,
									mod_magic_product,
