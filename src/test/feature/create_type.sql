...
--create sum aggregate functions，NODE:default initcond is NULL
--CREATE FUNCTION vec_accum_fin(int8) returns int8 as 'vexecutor.so' language c immutable strict;
...
--CREATE FUNCTION vec_avg_fin(bytea) returns numeric as 'vexecutor.so' language c immutable strict;
--CREATE FUNCTION vec_avg_prelim(bytea, bytea) returns bytea as 'vexecutor.so' language c immutable;
...
--CREATE AGGREGATE avg(vint4) ( 
--    sfunc = vint4_avg_accum,
--    prefunc = vec_avg_prelim,
--    finalfunc = vec_avg_fin,
--    stype = bytea);

    
--create count aggregate functions
--CREATE FUNCTION vec_inc_fin(bytea) returns int8 as 'vexecutor.so' language c immutable strict;
...
--change the name to veccount, urgly...because I don't want replace this agg functions in vcheck.c
--if we create a new count(*), then how can we drop it? the system prohibit to drop count(*) default.
