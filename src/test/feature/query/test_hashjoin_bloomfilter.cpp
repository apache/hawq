...
TEST_F(TestHashJoinBloomFilter, BasicTest)
{
    ...
    util.execute("create table fact(c1 int) WITH(appendonly=true, ORIENTATION=parquet) distributed by (c1);");
    util.execute("insert into fact select generate_series(1, 300000);");
