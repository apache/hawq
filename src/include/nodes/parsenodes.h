 ...
 *
 * $PostgreSQL: pgsql/src/include/nodes/parsenodes.h,v 1.334 2006/11/05 22:42:10
 *tgl Exp $
 *
 *-------------------------------------------------------------------------
 */
...
#define ACL_USAGE                                                             \
  (1 << 8)                        /* for languages, namespaces, FDWs, servers \
...
/*
 * Query -
 *	  Parse analysis turns all statements into a Query tree (via
 *transformStmt)
...
typedef struct Query {
  ...
  /* TODO Eventually we should remove these 4 members because they're not used
   * here.
   *      Instead they're in PlannedStmt.  However, we need to read old formats,
   * e.g.
 ...
 *
 *	Many of the node types used in raw parsetrees include a "location"
 *field.
 *	This is a byte (not character) offset in the original source text, to be
 ...
 *for
 *	  dropped columns.	When building an RTE we insert empty strings
 *("") for
 *	  dropped columns.	Note however that a stored rule may have
 *nonempty
 *	  colnames for columns dropped since the rule was created (and for that
 *	  matter the colnames might be out of date due to column renamings).
 *	  The same comments apply to FUNCTION RTEs when the function's return
 *type
 ...
 *	  was created.	(This is only possible for columns not actually
 *referenced
 *	  in the rule.)  When loading a stored rule, we replace the
 *joinaliasvars
 ...
 *
 *		Some of them require a little bit of transformation (which is
 *also
 *		done by transformStmt). The whole structure is then passed on to
 *		ProcessUtility (by-passing the optimization step) as the
 *utilityStmt
 *		field in Query.
 *****************************************************************************/
...
typedef struct AlterTableStmt {
  ...
  bool greenWay;    // allow for pluggable storage partition case
} AlterTableStmt;
...
typedef struct AlterPartitionCmd /* one subcmd of an ALTER TABLE...PARTITION */
  ...
  List *newpart_aosegnos; /* AO 'seg' number for the new partitions to be insert
                             into */
  char *format;           // pluggable storage format
} AlterPartitionCmd;
...
typedef struct CopyStmt {
  ...
  List *planTree;           /* the scan plan to copy for external table */
} CopyStmt;
 ...
 * output.	After parse analysis it's set to null, and the Query points to
 *the
...
typedef struct VacuumStmt {
  ...
  /*
   * OIDs for relfilenode, the internal heap and index relfilenodes for a bitmap
   * index.
 ...
 * ----------------------
 */
typedef struct CheckPointStmt { NodeTag type; } CheckPointStmt;
